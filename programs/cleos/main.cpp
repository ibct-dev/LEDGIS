...
string default_wallet_url = "unix://" + (determine_home_directory() / "led-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
struct register_interior_subcommand
{
   ...
   register_interior_subcommand(CLI::App *actionRoot)
   {
      ...
      register_interior->set_callback([this] {
         ...
         fc::variant regprod_var = fc::mutable_variant_object()("interior", producer_str)("producer_key", producer_key)("election_promise", election_promise)("url", url)("location", loc)("city", city)("logo_256", logo);
...
struct register_frontier_subcommand
{
   ...
   register_frontier_subcommand(CLI::App *actionRoot)
   {
      ...
      register_frontier->set_callback([this] {
         ...
         EOS_RETHROW_EXCEPTIONS(public_key_type_exception, "Invalid producer public key: ${public_key}", ("public_key", producer_key_str))
         fc::variant regprod_var = fc::mutable_variant_object()("frontier", producer_str)("producer_key", producer_key)("transfer_ratio", to_dapp_asset(N(led.token), transfer_ratio))("category", category)("url", url)("location", loc)("city", city)("logo_256", logo);
         auto accountPermissions = get_account_permissions(tx_permission, {producer_str, config::active_name});
...
struct change_ratio_subcommand
{
   ...
   change_ratio_subcommand(CLI::App *actionRoot)
   {
      ...
      change_ratio->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()("frontier", producer_str)("transfer_ratio", to_dapp_asset(N(led.token), transfer_ratio));
         auto accountPermissions = get_account_permissions(tx_permission, {producer_str, config::active_name});
...
struct vote_producer_proxy_subcommand
{
   ...
   vote_producer_proxy_subcommand(CLI::App *actionRoot)
   {
      ...
      vote_proxy->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()("voter", voter_str)("proxy", proxy_str)("interiors", std::vector<account_name>{});
...
struct vote_producers_subcommand
{
   ...
   vote_producers_subcommand(CLI::App *actionRoot)
   {
      ...
      vote_producers->set_callback([this] {
         ...
         fc::variant act_payload = fc::mutable_variant_object()("voter", voter_str)("proxy", "")("interiors", producer_names);
         auto accountPermissions = get_account_permissions(tx_permission, {voter_str, config::active_name});
         send_actions({create_action(accountPermissions, config::system_account_name, N(voteproducer), act_payload)});
      });
   }
...
struct approve_producer_subcommand
{
   ...
   approve_producer_subcommand(CLI::App *actionRoot)
   {
      ...
      approve_producer->set_callback([this] {
         ...
         fc::variant act_payload = fc::mutable_variant_object()("voter", voter)("proxy", "")("interiors", prods);
...
struct buyservice_subcommand
{
   ...
   buyservice_subcommand(CLI::App *actionRoot)
   {
      ...
      buy_service->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()("buyer", buyer_str)("quantity", to_asset(pay_quantity))("frontier", producer_name);
...
struct list_interiors_subcommand
{
   ...
   list_interiors_subcommand(CLI::App *actionRoot)
   {
      ...
      list_interiors->set_callback([this] {
         auto rawResult = call(get_interiors_func, fc::mutable_variant_object("json", true)("lower_bound", lower)("limit", limit));
         if (print_json)
         {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
...
struct list_frontiers_subcommand
{
   ...
   list_frontiers_subcommand(CLI::App *actionRoot)
   {
      ...
      list_frontiers->set_callback([this] {
         auto rawResult = call(get_frontiers_func, fc::mutable_variant_object("json", true)("lower_bound", lower)("limit", limit));
         if (print_json)
         {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
...
struct regproxy_subcommand
{
   ...
   regproxy_subcommand(CLI::App *actionRoot)
   {
      ...
      register_proxy->add_option("location", location, localized("a location representing a proxy"), true);
      ...
      register_proxy->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()("proxy", proxy)("slogan", slogan)("background", background)("url", url)("location", location)("logo_256", logo_256);
...
struct unregproxy_subcommand
{
   ...
   unregproxy_subcommand(CLI::App *actionRoot)
   {
      ...
      unregister_proxy->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()("proxy", proxy);
         auto accountPermissions = get_account_permissions(tx_permission, {proxy, config::active_name});
...
int main(int argc, char **argv)
{
   ...
   */
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "led.msig", "propose", variant_to_bin(N(led.msig), N(propose), args)}});
   });
   ...
   review->set_callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)("code", "led.msig")("scope", proposer)("table", "proposal")("table_key", "")("lower_bound", name(proposal_name).value)("upper_bound", name(proposal_name).value + 1)
                                // Less than ideal upper_bound usage preserved so cleos can still work with old buggy nodeos versions
      ...
      if (show_approvals_in_multisig_review)
      {
         ...
         try
         {
            const auto &result2 = call(get_table_func, fc::mutable_variant_object("json", true)("code", "led.msig")("scope", proposer)("table", "approvals2")("table_key", "")("lower_bound", name(proposal_name).value)("upper_bound", name(proposal_name).value + 1)
                                       // Less than ideal upper_bound usage preserved so cleos can still work with old buggy nodeos versions
                                       // Change to name(proposal_name).value when cleos no longer needs to support nodeos versions older than 1.5.0
                                       ("limit", 1));
         ...
         {
            const auto result3 = call(get_table_func, fc::mutable_variant_object("json", true)("code", "led.msig")("scope", proposer)("table", "approvals")("table_key", "")("lower_bound", name(proposal_name).value)("upper_bound", name(proposal_name).value + 1)
                                      // Less than ideal upper_bound usage preserved so cleos can still work with old buggy nodeos versions
                                      // Change to name(proposal_name).value when cleos no longer needs to support nodeos versions older than 1.5.0
                                      ("limit", 1));
         ...
         if (new_multisig)
         {
            ...
            {
               const auto result4 = call(get_table_func, fc::mutable_variant_object("json", true)("code", "led.msig")("scope", "led.msig")("table", "invals")("table_key", "")("lower_bound", a.first.value)("upper_bound", a.first.value + 1)
                                         // Less than ideal upper_bound usage preserved so cleos can still work with old buggy nodeos versions
                                         // Change to name(proposal_name).value when cleos no longer needs to support nodeos versions older than 1.5.0
                                         ("limit", 1));
   ...
   auto approve_or_unapprove = [&](const string &action) {
      ...
      send_actions({chain::action{accountPermissions, "led.msig", action, variant_to_bin(N(led.msig), action, args)}});
   };
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "led.msig", "invalidate", variant_to_bin(N(led.msig), "invalidate", args)}});
   });
   ...
   cancel->set_callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, "led.msig", "cancel", variant_to_bin(N(led.msig), N(cancel), args)}});
   });
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "led.msig", "exec", variant_to_bin(N(led.msig), N(exec), args)}});
   });
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send led.system contract action to the blockchain."), false);
