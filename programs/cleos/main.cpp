...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(led.msig), N(propose), variant_to_bin( N(led.msig), N(propose), args ) }});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(led.msig), name(action), variant_to_bin( N(led.msig), name(action), args ) }});
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(led.msig), N(invalidate), variant_to_bin( N(led.msig), N(invalidate), args ) }});
   });
   ...
   cancel->callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, N(led.msig), N(cancel), variant_to_bin( N(led.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(led.msig), N(exec), variant_to_bin( N(led.msig), N(exec), args ) }});
      }
   );
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send led.system contract action to the blockchain."));
