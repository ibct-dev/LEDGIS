...
function usage() {
  ...
  -i DIR      Directory where ledgis is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the ledgis data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export LEDGIS_INSTALL_DIR=${INSTALL_LOCATION:-$LEDGIS_INSTALL_DIR}

if [[ ! -d "${LEDGIS_INSTALL_DIR}" ]]; then
   echo "[LEDGIS installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${LEDGIS_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/EOSIO/eos/issues/6940
   echo "[LEDGIS installation found: ${LEDGIS_INSTALL_DIR}]" && INSTALL_PATHS+=("${LEDGIS_INSTALL_DIR}") # LEDGIS_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${LEDGIS_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/ledgis")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/ledgis")
            fi
            # Version < 1.8.0; Before we started using ~/ledgis/1.8.x
            ...
            echo " - LEDGIS Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled LEDGIS Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing LEDGIS Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/ledgis_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
