...
If you want instead of `eosio.bios`, a more elaborate system smart contract like `led.system`, you have to build it first and then execute the following command from a terminal (or from the shell). Make sure you replace the `EOSIO_SYSTEM_CONTRACTS_DIRECTORY` with the directory path where you have built the `led.system` system contract, the directory path where `eoios.system.abi` and `led.system.wasm` are located.

```sh
cleos set contract eosio EOSIO_SYSTEM_CONTRACTS_DIRECTORY/led.system/ led.system.wasm led.system.abi
```
