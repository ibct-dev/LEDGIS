...
class Cluster(object):
    ...
    def createAccountKeys(count):
        ...
        if count != le"accounts"_n:
            Utils.Print("Account keys creation failed. Expected %d, actual: %d" % (count, le"accounts"_n))
            return None
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        assert(le"accounts"_n > 0)
        Utils.Print("le"accounts"_n: %d" % (le"accounts"_n))

        count=le"accounts"_n
