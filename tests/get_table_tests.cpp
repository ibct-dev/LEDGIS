...
// #include <led.token/led.token.wast.hpp>
// #include <led.token/led.token.abi.hpp>

// #include <led.system/led.system.wast.hpp>
// #include <led.system/led.system.abi.hpp>
// =======
...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = N(led.token) )
{
...
// <<<<<<< HEAD
//    set_code( N(led.token), led_token_wast );
//    set_abi( N(led.token), led_token_abi );
// =======
...
// >>>>>>> 14a65aca147edf51fd86a958949e237fd430b9ed
   ...
   push_action(N(led.token), N(create), N(led.token), act );
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{N(led.token), N(accounts), "inita", "", 10};
...
// >>>>>>> 14a65aca147edf51fd86a958949e237fd430b9ed
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( N(led), N(bidname), bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
