...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = "led.token"_n )
{
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_scope_test, TESTER_T, backing_store_ts) { try {
   ...
   t.create_accounts({ "led.token"_n, "led.ram"_n, "led.ramfee"_n, "led.stake"_n,
   ...
   t.set_code( "led.token"_n, contracts::led_token_wasm() );
   t.set_abi( "led.token"_n, contracts::led_token_abi().data() );
   t.produce_blocks(1);
   ...
   t.push_action("led.token"_n, "create"_n, "led.token"_n, act );
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{"led.token"_n, "accounts"_n, "inita", "", 10};
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_table_test, TESTER_T, backing_store_ts) { try {
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("led", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("led", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("led", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("led", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_AUTO_TEST_CASE_TEMPLATE( get_table_by_seckey_test, TESTER_T, backing_store_ts) { try {
   ...
   t.set_code( config::system_account_name, contracts::led_system_wasm() );
   t.set_abi( config::system_account_name, contracts::led_system_abi().data() );
   ...
   auto bidname = [&t]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return t.push_action( "led"_n, "bidname"_n, bidder, fc::mutable_variant_object()
                           ("bidder",  bidder)
                           ("newname", newname)
                           ("bid", bid)
                            );
