...
asset get_currency_balance(const TESTER& chain, account_name account) {
   return chain.get_currency_balance("led.token"_n, symbol(SY(4,CUR)), account);
}

const std::string led_token = name("led.token"_n).to_string();
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.create_account("led.token"_n);
   chain.produce_blocks(10);

   chain.set_code("led.token"_n, contracts::led_token_wasm());
   chain.set_abi("led.token"_n, contracts::led_token_abi().data());
   ...
   chain.push_action(config::system_account_name, linkauth::get_name(), tester_account, fc::mutable_variant_object()
           ...
           ("requirement", "first"));
   chain.produce_blocks();
   chain.push_action("led.token"_n, "create"_n, "led.token"_n, mutable_variant_object()
           ("issuer", led_token)
           ("maximum_supply", "9000000.0000 CUR")
   );


   chain.push_action("led.token"_n, name("issue"), "led.token"_n, fc::mutable_variant_object()
           ("to",       led_token)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action("led.token"_n, name("transfer"), "led.token"_n, fc::mutable_variant_object()
       ("from", led_token)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, "led.token"_n);
   ...
   trace = chain.push_action("led.token"_n, name("transfer"), "tester"_n, fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, "led.token"_n);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, "tester"_n);
...
BOOST_AUTO_TEST_CASE(delete_auth_test) { try {
   ...
   // transfer from led.token to tester
   trace = chain.push_action("led.token"_n, name("transfer"), "led.token"_n, fc::mutable_variant_object()
       ("from", "led.token")
       ("to", "tester")
       ("quantity", "100.0000 CUR")
       ("memo", "hi" )
...
BOOST_AUTO_TEST_CASE( mindelay_test ) { try {
   ...
   const auto& acnt = chain.control->db().get<account_object,by_name>("led.token"_n);
   ...
   const auto a = chain.control->db().get<account_object,by_name>("led.token"_n).get_abi();

   string action_type_name = abis.get_action_type(name("transfer"));

   action act;
   act.account = "led.token"_n;
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action("led.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, "first"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action("led.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, "second"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action("led.token"_n, name("transfer"), vector<permission_level>{{"tester"_n, config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
