...
BOOST_AUTO_TEST_CASE(test_deltas_resources_history) {
   for (backing_store_type backing_store : { backing_store_type::CHAINBASE, backing_store_type::ROCKSDB }) {
      ...
      chain.create_accounts({ "led.token"_n, "led.ram"_n, "led.ramfee"_n, "led.stake"_n});
      ...
      chain.push_action("led.token"_n, "issue"_n, "led.token"_n, fc::mutable_variant_object()
         ("to",       "eosio")
      ...
      chain.set_code( config::system_account_name, contracts::led_system_wasm() );
      chain.set_abi( config::system_account_name, contracts::led_system_abi().data() );
