...
namespace led_system {

class led_system_tester : public TESTER {
public:

   led_system_tester()
   : led_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   led_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ "led.token"_n, "led.ram"_n, "led.ramfee"_n, "led.stake"_n,
               "led.bpay"_n, "led.vpay"_n, "led.saving"_n, "led.names"_n });

      produce_blocks( 100 );

      set_code( "led.token"_n, contracts::led_token_wasm() );
      set_abi( "led.token"_n, contracts::led_token_abi().data() );

      {
         const auto& accnt = control->db().get<account_object,by_name>( "led.token"_n );
         abi_def abi;
         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
         token_abi_ser.set_abi(abi, abi_serializer::create_yield_function( abi_serializer_max_time ));
      }

      create_currency( "led.token"_n, config::system_account_name, core_from_string("10000000000.0000") );
      issue(config::system_account_name,      core_from_string("1000000000.0000"));
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("led") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("led")) + get_balance(name("led.ramfee")) + get_balance(name("led.stake")) + get_balance(name("led.ram")) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( "led.token"_n, act, "accounts"_n, name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "led.token"_n, "issue"_n, manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( "led.token"_n, "transfer"_n, manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( "led.token"_n, name(symbol_code), "stat"_n, name(symbol_code) );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( "led.msig"_n, config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( name("led"), name("led.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( "led.msig"_n, contracts::led_msig_wasm() );
         set_abi( "led.msig"_n, contracts::led_msig_abi().data() );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( "led.msig"_n );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( name("led"), name("alice1111111"), core_from_string("650000000.0000"), name("led") );
