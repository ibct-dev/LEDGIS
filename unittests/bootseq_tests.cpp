...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance("led.token"_n, symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        create_accounts({"led.msig"_n, "led.token"_n, "led.ram"_n, "led.ramfee"_n, "led.stake"_n, "led.vpay"_n, "led.bpay"_n, "led.saving"_n });
        // Set code for the following accounts:
        //  - eosio (code: led.bios) (already set by tester constructor)
        //  - led.msig (code: led.msig)
        //  - led.token (code: led.token)
        // set_code_abi("led.msig"_n, contracts::led_msig_wasm(), contracts::led_msig_abi().data());//, &eosio_active_pk);
        // set_code_abi("led.token"_n, contracts::led_token_wasm(), contracts::led_token_abi().data()); //, &eosio_active_pk);

        set_code_abi("led.msig"_n,
                     contracts::led_msig_wasm(),
                     contracts::led_msig_abi().data());//, &eosio_active_pk);
        set_code_abi("led.token"_n,
                     contracts::led_token_wasm(),
                     contracts::led_token_abi().data()); //, &eosio_active_pk);

        // Set privileged for led.msig and led.token
        set_privileged("led.msig"_n);
        set_privileged("led.token"_n);

        // Verify led.msig and led.token is privileged
        const auto& led_msig_acc = get<account_metadata_object, by_name>("led.msig"_n);
        BOOST_TEST(led_msig_acc.is_privileged() == true);
        const auto& led_token_acc = get<account_metadata_object, by_name>("led.token"_n);
        BOOST_TEST(led_token_acc.is_privileged() == true);


        // Create SYS tokens in led.token, set its manager as eosio
        ...
        create_currency("led.token"_n, config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to led.system
        issue("led.token"_n, config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth("led.stake"_n, a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == name("led"));
