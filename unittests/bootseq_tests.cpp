...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // set_code_abi(N(led.msig), contracts::led_msig_wasm(), contracts::led_msig_abi().data());//, &eosio_active_pk);
        // set_code_abi(N(led.token), contracts::led_token_wasm(), contracts::led_token_abi().data()); //, &eosio_active_pk);
