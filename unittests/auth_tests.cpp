...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   chain.link_authority("alice", "led", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "led", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with led reqauth
   chain.unlink_authority("alice", "led", "reqauth");
   ...
   chain.link_authority("alice", "led", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "led", "first",  "reqauth");
...
try {
   ...
   chain.create_account("led.test1");

   // Creating account with led. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("led.test2", "joe"), action_validate_exception,
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "led", "led.any", "reqauth" );
   chain.link_authority( "bob", "led", "led.any", "reqauth" );
