...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for led reqauth action with alice's spending key
   chain.link_authority(name("alice"), name("led"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("led"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with led reqauth
   chain.unlink_authority(name("alice"), name("led"), name("reqauth"));
   ...
   // Link authority for any led action with alice's scud key
   chain.link_authority(name("alice"), name("led"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("led"), name("first"), name("reqauth"));
...
try {
   ...
   // Creating account with led. prefix with privileged account
   chain.create_account(name("led.test1"));

   // Creating account with led. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("led.test2"), name("joe")), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'led.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("led"), name("led.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("led"), name("led.any"), name("reqauth") );
