...
class kv_tester : public tester {
 public:
   ...
   void test_max_iterators() {
      ...
      // individual limits
      BOOST_TEST(iterlimit({{"led.kvram"_n, 7, false}}) == "");
      BOOST_TEST(iterlimit({{"led.kvram"_n, 2000, false}}) == "Too many iterators");
      // erase iterators and create more
      BOOST_TEST(iterlimit({{"led.kvram"_n, 6, false},
                            {"led.kvram"_n, 2, true},
                            {"led.kvram"_n, 3, false}}) == "");
      BOOST_TEST(iterlimit({{"led.kvram"_n, 6, false},
                            {"led.kvram"_n, 2, true},
                            {"led.kvram"_n, 4, false}}) == "Too many iterators");
      ...
      // BOOST_TEST_REQUIRE(set_kv_limits("led.kvram"_n, 1024, 1024, 0xFFFFFFFF) == "");
      // BOOST_TEST_REQUIRE(set_kv_limits("led.kvram"_n, 1024, 1024, 0xFFFFFFFF) == "");
      // BOOST_TEST(iterlimit({{"led.kvram"_n, 0xFFFFFFFF, false}, {"led.kvram"_n, 1, false}}) == "Too many iterators");
   }
...
constexpr name databases[] = { "led.kvram"_n };
...
BOOST_DATA_TEST_CASE_F(tester, notify, bdata::make(databases), db) {
   ...
   push_action( "led"_n, "setpriv"_n, "led"_n, mutable_variant_object()("account", "setup"_n)("is_priv", 1));
