...
public:
   ...
   legis_token_tester() {
      ...
      create_accounts( { N(alice), N(bob), N(carol), N(legis.token) } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = eosio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(legis.token), acc, N(accounts), symbol_code );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(legis.token), N(create), mvo()
           ( "issuer", issuer)
...
BOOST_FIXTURE_TEST_CASE( create_tests, legis_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, legis_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, legis_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, legis_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, legis_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, legis_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, legis_token_tester ) try {
