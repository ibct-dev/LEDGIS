...
BOOST_AUTO_TEST_CASE( activate_preactivate_feature ) try {
   ...
   BOOST_CHECK_EXCEPTION( c.set_code( config::system_account_name, contracts::led_bios_wasm() ),
                          wasm_exception, fc_exception_message_is("env.is_feature_activated unresolveable")
   );

   // But the old bios contract can still be set.
   c.set_code( config::system_account_name, contracts::before_preactivate_led_bios_wasm() );
   c.set_abi( config::system_account_name, contracts::before_preactivate_led_bios_abi().data() );
...
BOOST_AUTO_TEST_CASE( fix_linkauth_restriction ) { try {
   ...
   validate_disallow("led", "linkauth");
   validate_disallow("led", "unlinkauth");
   validate_disallow("led", "deleteauth");
   validate_disallow("led", "updateauth");
   validate_disallow("led", "canceldelay");
...
BOOST_AUTO_TEST_CASE( producer_schedule_change_extension_test ) { try {
   ...
   { // ensure producer_schedule_change_extension is rejected
      ...
      bad_block->producer_signature = remote.get_private_key("led"_n, "active").sign(sig_digest);
...
BOOST_AUTO_TEST_CASE( wtmsig_block_signing_inflight_legacy_test ) { try {
   ...
   c.preactivate_protocol_features( {*d} );
   vector<legacy::producer_key> sched = {{"led"_n, c.get_public_key("led"_n, "bsk")}};
   ...
   c.block_signing_private_keys.emplace(get_public_key("led"_n, "bsk"), get_private_key("led"_n, "bsk"));
   c.produce_block();
