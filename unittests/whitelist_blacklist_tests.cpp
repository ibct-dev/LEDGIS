...
class whitelist_blacklist_tester {
   public:
      ...
      void init( bool bootstrap = true ) {
         ...
         chain->create_accounts({N(legis.token), N(alice), N(bob), N(charlie)});
         chain->set_code(N(legis.token), contracts::legis_token_wasm() );
         chain->set_abi(N(legis.token), contracts::legis_token_abi().data() );
         chain->push_action( N(legis.token), N(create), N(legis.token), mvo()
              ( "issuer", "legis.token" )
              ( "maximum_supply", "1000000.00 TOK" )
         );
         chain->push_action( N(legis.token), N(issue), N(legis.token), mvo()
              ( "to", "legis.token" )
      ...
      transaction_trace_ptr transfer( account_name from, account_name to, string quantity = "1.00 TOK" ) {
         return chain->push_action( N(legis.token), N(transfer), from, mvo()
            ( "from", from )
            ( "to", to )
            ( "quantity", quantity )
            ( "memo", "" )
...
BOOST_AUTO_TEST_CASE( actor_whitelist ) { try {
   ...
   test.actor_whitelist = {config::system_account_name, N(legis.token), N(alice)};
   test.init();

   test.transfer( N(legis.token), N(alice), "1000.00 TOK" );
...
BOOST_AUTO_TEST_CASE( contract_whitelist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(legis.token), N(bob)};
   test.init();

   test.transfer( N(legis.token), N(alice), "1000.00 TOK" );

   test.transfer( N(alice), N(legis.token) );
   ...
   test.chain->set_code(N(bob), contracts::legis_token_wasm() );
   test.chain->set_abi(N(bob), contracts::legis_token_abi().data() );

   test.chain->produce_blocks();

   test.chain->set_code(N(charlie), contracts::legis_token_wasm() );
   test.chain->set_abi(N(charlie), contracts::legis_token_abi().data() );
...
BOOST_AUTO_TEST_CASE( action_blacklist ) { try {
   whitelist_blacklist_tester<> test;
   test.contract_whitelist = {config::system_account_name, N(legis.token), N(bob), N(charlie)};
...
BOOST_AUTO_TEST_CASE( blacklist_eosio ) { try {
   ...
   tester1.chain->set_code(config::system_account_name, contracts::legis_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
