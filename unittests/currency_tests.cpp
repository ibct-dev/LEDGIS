...
class currency_tester : public TESTER {
   public:
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::led_token_abi().data()).as<abi_def>(), abi_serializer_max_time)
      {
      ...
      static const std::string led_token;
};

const std::string currency_tester::led_token = name(N(led.token)).to_string();
