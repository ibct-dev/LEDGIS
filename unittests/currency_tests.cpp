...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance("led.token"_n, symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action("led.token"_n, "issue"_n, mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
         :TESTER(),abi_ser(json::from_string(contracts::led_token_abi().data()).as<abi_def>(), abi_serializer::create_yield_function( abi_serializer_max_time ))
      {
         create_account( "led.token"_n);
         set_code( "led.token"_n, contracts::led_token_wasm() );

         auto result = push_action("led.token"_n, "create"_n, mutable_variant_object()
                 ("issuer",       led_token)
         ...
         result = push_action("led.token"_n, "issue"_n, mutable_variant_object()
                 ("to",       led_token)
      ...
      static const name led_token;
};

const name currency_tester::led_token = "led.token"_n;
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance("led.token"_n, expected.get_symbol(), "led.token"_n);
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action("led.token"_n, "transfer"_n, mutable_variant_object()
         ("from", led_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action("led.token"_n, "transfer"_n, mutable_variant_object()
                                    ("from", led_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(led_token, "alice"_n, "100.0000 CUR");
