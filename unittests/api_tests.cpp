...
BOOST_AUTO_TEST_CASE_TEMPLATE(action_receipt_tests, TESTER_T, backing_store_ts) { try {
   ...
   t.set_code( config::system_account_name, contracts::led_bios_wasm() );

   t.set_code( "test"_n, contracts::led_bios_wasm() );
   t.set_abi( "test"_n, contracts::led_bios_abi().data() );
   t.set_code( "test"_n, contracts::payloadless_wasm() );
...
BOOST_AUTO_TEST_CASE_TEMPLATE(deferred_transaction_tests, TESTER_T, backing_store_ts) { try {
...

   {
      ...
      // fail because testapi@additional permission is not unilaterally satisfied by testapi@led.code.
      dtt_action dtt_act3;
