...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {"led.token"_n} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( "led.token"_n, contracts::led_token_wasm() );
   c.set_abi( "led.token"_n, contracts::led_token_abi().data() );
   c.produce_blocks(10);


   auto cr = c.push_action( "led.token"_n, "create"_n, "led.token"_n, mutable_variant_object()
              ("issuer",       "led" )
              ("maximum_supply", core_from_string("10000000.0000"))
      );

   cr = c.push_action( "led.token"_n, "issue"_n, config::system_account_name, mutable_variant_object()
              ("to",       "led" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
      );

   cr = c.push_action( "led.token"_n, "transfer"_n, config::system_account_name, mutable_variant_object()
              ("from",     "led")
              ("to",       "dan" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
