...
read_only::get_interiors_result read_only::get_interiors( const read_only::get_interiors_params& p ) const try {
   ...
   const auto* const secondary_table_id = d.find<chain::table_id_object, chain::by_code_scope_table>(
           boost::make_tuple(config::system_account_name, config::system_account_name, name(N(interiors).to_uint64_t() | secondary_index_num)));
   ...
   for( ; it != secondary_index_by_secondary.end() && it->t_id == secondary_table_id->id; ++it ) {
      ...
      copy_inline_row(*kv_index.find(boost::make_tuple(table_id->id, it->primary_key)), data);
      if (p.json)
         result.rows.emplace_back( abis.binary_to_variant( abis.get_table_type(N(interiors)), data, abi_serializer::create_yield_function( abi_serializer_max_time ), shorten_abi_errors ) );
      else
         result.rows.emplace_back(fc::variant(data));
   }
...
read_only::get_frontiers_result read_only::get_frontiers( const read_only::get_frontiers_params& p ) const try {
   ...
   const auto* const secondary_table_id = d.find<chain::table_id_object, chain::by_code_scope_table>(
           boost::make_tuple(config::system_account_name, config::system_account_name, name(N(frontiers).to_uint64_t() | secondary_index_num)));
   ...
   for( ; it != secondary_index_by_secondary.end() && it->t_id == secondary_table_id->id; ++it ) {
      ...
      copy_inline_row(*kv_index.find(boost::make_tuple(table_id->id, it->primary_key)), data);
      if (p.json)
         result.rows.emplace_back( abis.binary_to_variant( abis.get_table_type(N(frontiers)), data, abi_serializer::create_yield_function( abi_serializer_max_time ), shorten_abi_errors ) );
      else
         result.rows.emplace_back(fc::variant(data));
   }
   return result;
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      const auto token_code = N(led.token);
...
chain::symbol read_only::extract_core_symbol()const {
   ...
   const auto& d = db.db();
   const auto* t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( N(led), N(led), N(rammarket) ));
   if( t_id != nullptr ) {
      const auto &idx = d.get_index<key_value_index, by_scope_primary>();
